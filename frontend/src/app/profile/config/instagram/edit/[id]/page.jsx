...
const EditConfig = ({params}) => {
    ...
    const get_config = async () => {
        await ConfigAPI.getInstagramConfig(params.id, sessionStorage.getItem("access-token")).then(res => {
            setConfig(res.data)
        }).catch(error => {
            setShowMessage(true)
    ...
    const onSubmitConfig = async (event) => {
        ...
        if (action === "update") {
            ...
            await ConfigAPI.updateInstagramConfig(data, sessionStorage.getItem("access-token")).then(res => {
                setShowMessage(true)
                setMessage("Instagram config was successfully updated!")
                setSeverity("success")
            ...
            await ConfigAPI.newInstagramConfig(data, sessionStorage.getItem("access-token")).then(res => {
                setShowMessage(true)
                setMessage("Instagram config was successfully created!")
                setSeverity("success")
                setIsUpdateComplete(true)
            }).catch(error => {
                setShowMessage(true)
        ...
        <Box>
            <Box m={0} p={2}>
                <Button sx={{marginBottom: 2}} variant="contained" startIcon={<ArrowLeftIcon/>}
                        onClick={() => router.push("/profile/config/instagram")}>
                    Back to config list
                </Button>
                {showMessage &&
